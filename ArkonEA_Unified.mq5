...
//|              Unified merge of ARKON_MT5_EA / Arkon45EA / ArkonExpert |
//+------------------------------------------------------------------+
#property copyright "Arkon Quant"
#property link      "https://arkon.quant"
#property version   "51.01"

#include <Trade\Trade.mqh>

input string WebhookURL = "http://127.0.0.1:3000"; 
input string WebhookSecret = "Ashjambi@4794";
...
input string      GOLD_Header_Marker        = "===================="; // ==== GOLD (XAUUSD) Settings ====
input bool        EnableGoldTrading          = true;     // Enable GOLD (XAUUSD) trading
input double      GoldMaxRiskPerTrade        = 0.5;      // Max risk per GOLD trade (% of equity)
input int         GoldMaxConcurrentPositions = 5;        // Max concurrent GOLD positions
input double      GoldSpreadFilter           = 50.0;     // Max spread in points for GOLD
input bool        GoldSessionFilter          = true;     // Enable session filter for GOLD
input string      GoldSessionStart           = "08:00";  // GOLD session start (UTC)
input string      GoldSessionEnd             = "17:00";  // GOLD session end (UTC)
input double      GoldFixedLotSize           = 0.01;     // Base lot size for GOLD
...
string g_WebhookURL;
CTrade trade;
bool g_IsZombieBuy = false;   // Legacy aggregate flag (dashboard only)
bool g_IsZombieSell = false;  // Legacy aggregate flag (dashboard only)

// FIX v51.01: Per-symbol zombie tracking to prevent cross-symbol blocking
// (e.g. XAUUSD zombie basket should NOT block BTCUSD entries)
string g_ZombieSymbols[100];
bool   g_ZombieBuyPerSymbol[100];
bool   g_ZombieSellPerSymbol[100];
int    g_ZombieSymbolCount = 0;
...
int OnInit() {
    ...
    Print("Bridge URL configured to: ", g_WebhookURL);
    Print("==========================================================");
    Print("ARKON EA Initialized. VERSION 51.01 (PER-SYMBOL ZOMBIE FIX)");
...
void UpdateDashboard() {
    string dashboard = "=== ARKON EXPERT v51.01 ===\n";
    ...
    // Show per-symbol zombie status on dashboard
    if(g_ZombieSymbolCount > 0) {
        dashboard += "\n--- ZOMBIE STATUS (PER-SYMBOL) ---\n";
        for(int s = 0; s < g_ZombieSymbolCount && s < 100; s++) {
            if(g_ZombieBuyPerSymbol[s] || g_ZombieSellPerSymbol[s]) {
                dashboard += g_ZombieSymbols[s] + ": Buy=" + (g_ZombieBuyPerSymbol[s] ? "ZOMBIE" : "OK") + " | Sell=" + (g_ZombieSellPerSymbol[s] ? "ZOMBIE" : "OK") + "\n";
            }
        }
    }
...
void ManageCapitalRecovery() {
    ...
    // FIX v51.01: Zombie status is now tracked PER-SYMBOL to prevent cross-symbol blocking
    // (e.g. XAUUSD zombie basket should NOT block BTCUSD entries)
    
    // Dynamic Zombie Basket criteria based on account balance percentage (10%)
    // Floor: MathMax(500, balance*10%) ensures small accounts get reasonable protection
    // Cap: floor never exceeds 25% of current balance (protects very small accounts from over-strict floor)
    double acctBalance = AccountInfoDouble(ACCOUNT_BALANCE);
    double maxDrawdownAllowance = acctBalance * 0.10; // 10% of balance
    double floorLimit = MathMax(500.0, acctBalance * 0.10);
    if(floorLimit > acctBalance * 0.25) floorLimit = acctBalance * 0.25;
    if(maxDrawdownAllowance < floorLimit) maxDrawdownAllowance = floorLimit;

    // Aggregate (legacy) counters - kept for dashboard/logging only
    double buyFloatingPnL = 0, sellFloatingPnL = 0;
    double buyVolume = 0, sellVolume = 0;
    int buyCount = 0, sellCount = 0;
    
    // Per-symbol zombie tracking
    string zombieSymbols[100];
    bool   zombieBuyPerSymbol[100];
    bool   zombieSellPerSymbol[100];
    int    zombieSymbolCount = 0;
    ...
    for(int i = PositionsTotal() - 1; i >= 0; i--) {
        ...
        int symIdx = -1;
        for(int s = 0; s < zombieSymbolCount; s++) {
            if(zombieSymbols[s] == posSymbol) { symIdx = s; break; }
        }
        if(symIdx < 0 && zombieSymbolCount < 100) {
            symIdx = zombieSymbolCount;
            zombieSymbols[symIdx] = posSymbol;
            zombieSymbolCount++;
        }
    }
    
    // Second pass: compute per-symbol PnL & counts
    for(int s = 0; s < zombieSymbolCount; s++) {
        string sym = zombieSymbols[s];
        double symBuyPnL = 0, symSellPnL = 0;
        int symBuyCount = 0, symSellCount = 0;
        ...
        for(int i = PositionsTotal() - 1; i >= 0; i--) {
            ...
            if(type == POSITION_TYPE_BUY) { symBuyPnL += profit; symBuyCount++; }
            else { symSellPnL += profit; symSellCount++; }
        }
        
        zombieBuyPerSymbol[s] = (symBuyCount >= 5 && symBuyPnL < -maxDrawdownAllowance);
        zombieSellPerSymbol[s] = (symSellCount >= 5 && symSellPnL < -maxDrawdownAllowance);
        
        if(zombieBuyPerSymbol[s] || zombieSellPerSymbol[s]) {
            Print("🧟 ZOMBIE STATUS [", sym, "]: Buy=", zombieBuyPerSymbol[s], " (count=", symBuyCount, " pnl=", DoubleToString(symBuyPnL, 2), ") | Sell=", zombieSellPerSymbol[s], " (count=", symSellCount, " pnl=", DoubleToString(symSellPnL, 2), ")");
        }
    }
    
    // Legacy aggregate flags (kept for backward compat / dashboard)
    ...
    // Store per-symbol arrays globally for ProcessSignal to query
    g_ZombieSymbolCount = zombieSymbolCount;
    for(int s = 0; s < zombieSymbolCount && s < 100; s++) {
        g_ZombieSymbols[s] = zombieSymbols[s];
        g_ZombieBuyPerSymbol[s] = zombieBuyPerSymbol[s];
        g_ZombieSellPerSymbol[s] = zombieSellPerSymbol[s];
    }
}
...
void SendState() {
    ...
    // Live bid/ask for this chart's symbol, feeds server-side PreTradeRiskGuard staleness check
    string chartSymbol = Symbol();
    string canonicalSymbol = CanonicalizeSymbol(chartSymbol);
    double chartBid = SymbolInfoDouble(chartSymbol, SYMBOL_BID);
    double chartAsk = SymbolInfoDouble(chartSymbol, SYMBOL_ASK);
    long quoteTimestampMs = (long)TimeGMT() * 1000;
    string marketQuotesJson = "{\"" + canonicalSymbol + "\":{\"bid\":" + DoubleToString(chartBid, 5) + ",\"ask\":" + DoubleToString(chartAsk, 5) + ",\"timestamp\":" + (string)quoteTimestampMs + "}}";

    string payload = "{\"positions\":" + positionsJson + ",\"crl_baseline\":" + DoubleToString(g_AccumulatedClosedProfit, 2) + ",\"crl_current\":" + DoubleToString(g_CurrentClosedProfit, 2) + ",\"crl_diff\":" + DoubleToString(diff, 2) + ",\"crl_budget\":" + DoubleToString(budgetEst, 2) + ",\"crl_threshold\":" + DoubleToString(CRLIncrementThreshold, 2) + ",\"equity\":" + DoubleToString(accountEquity, 2) + ",\"margin\":" + DoubleToString(accountMargin, 2) + ",\"market_quotes\":" + marketQuotesJson + "}";
...
// Strips broker-specific suffixes (.pro, .a, .c, m) so the reported symbol matches the canonical asset name used server-side
string CanonicalizeSymbol(string sym) {
    string s = sym;
    int dotPos = StringFind(s, ".");
    if(dotPos >= 0) s = StringSubstr(s, 0, dotPos);
    int len = StringLen(s);
    if(len > 3 && StringSubstr(s, len - 1, 1) == "m") s = StringSubstr(s, 0, len - 1);
    StringToUpper(s);
    return s;
}
...
void ProcessSignal(string json) {
    ...
    if(totalSlices > 1) Print("🔪 [CHILD ORDER DISPATCH] Executing Slice ", (sliceIndex + 1), "/", totalSlices, " | Style: ", executionStyle, " | Route: ", routeHint);

    string rawSymbol = rawSymbolExtracted;
    if(StringLen(rawSymbol) == 0) rawSymbol = rawAsset;
    if(StringLen(rawSymbol) == 0) rawSymbol = ExtractJSONString(json, "original_symbol");
    direction = ExtractJSONString(json, "direction");
    lotSize = ExtractJSONDouble(json, "fixedLotSize");
    sl = ExtractJSONDouble(json, "stopLoss");
    ...
    // Bitcoin (BTC): +0.01
    // Ethereum (ETH): +0.1
    // GOLD (XAUUSD): +0.005 (lower volatility asset)
    ...
    bool isGoldSignal = (StringFind(rawSymbol, "XAU") >= 0) || (StringFind(rawSymbol, "GOLD") >= 0);
    
    // Check GOLD enable/disable
    if(isGoldSignal && !EnableGoldTrading) {
        Print("⏩ GOLD Trading Disabled: EnableGoldTrading=false. Skipping.");
        return;
    }
    
    string resolvedSymbol = ResolveSymbol(rawSymbol);
    
    // GOLD: Apply spread filter before anything else
    if(isGoldSignal) {
        double currentSpread = (double)SymbolInfoInteger(resolvedSymbol, SYMBOL_SPREAD);
        if(currentSpread > GoldSpreadFilter) {
            Print("⏩ GOLD Spread Filter: Spread ", DoubleToString(currentSpread, 0), " > ", DoubleToString(GoldSpreadFilter, 0), ". Skipping.");
            return;
        }
    }
    
    // GOLD: Apply session filter
    if(isGoldSignal && GoldSessionFilter) {
        MqlDateTime dt;
        TimeCurrent(dt);
        int currentHour = dt.hour;
        int startHour = (int)StringToInteger(StringSubstr(GoldSessionStart, 0, 2));
        int endHour = (int)StringToInteger(StringSubstr(GoldSessionEnd, 0, 2));
        bool inSession = (currentHour >= startHour && currentHour < endHour);
        if(!inSession) {
            Print("⏩ GOLD Session Filter: Outside trading hours (", GoldSessionStart, "-", GoldSessionEnd, " UTC). Skipping.");
            return;
        }
    }
    ...
    } else if(isGoldSignal) {
        scaledLot = lotSize + (increments * 0.005);
    ...
    // Asset-specific max lot caps
    if (isGoldSignal && lotSize > 0.5) {
        lotSize = 0.5;
        Print("⚠️ GOLD Lot capped to MAX 0.5 lots.");
    } else if (!isGoldSignal && lotSize > 1.0) {
        lotSize = 1.0;
    ...
    if(action == "ENTRY" || action == "HEDGE" || action == "FLIP") {
        // FIX v51.01: Check zombie status PER-SYMBOL so XAUUSD zombie doesn't block BTCUSD
        bool isZombieForThisSymbol = false;
        for(int s = 0; s < g_ZombieSymbolCount && s < 100; s++) {
            if(g_ZombieSymbols[s] == resolvedSymbol) {
                if(direction == "LONG" && g_ZombieBuyPerSymbol[s]) isZombieForThisSymbol = true;
                if(direction == "SHORT" && g_ZombieSellPerSymbol[s]) isZombieForThisSymbol = true;
                break;
            }
        }
        if(isZombieForThisSymbol) {
            Print("⏩ BLOCK ENTRY: ", resolvedSymbol, " ", direction, " basket is Zombie! Grid reinforcement blocked for THIS symbol only.");
            return;
        }
        
        // GOLD: Check max concurrent positions
        if(isGoldSignal) {
            int goldPositionCount = 0;
            for(int pi = PositionsTotal() - 1; pi >= 0; pi--) {
                ulong pt = PositionGetTicket(pi);
                if(pt > 0 && PositionGetString(POSITION_SYMBOL) == resolvedSymbol) {
                    goldPositionCount++;
                }
            }
            if(goldPositionCount >= GoldMaxConcurrentPositions) {
                Print("⏩ GOLD Max Positions Reached: ", goldPositionCount, " >= ", GoldMaxConcurrentPositions, ". Skipping.");
                return;
            }
        }
