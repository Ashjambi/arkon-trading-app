...
echo "=================================================="
for f in src/services/CoordinationTraceService.ts src/services/CoordinationTraceService.test.ts; do
  if [ -f "$f" ]; then
    echo "EXISTS: $f"
  else
    echo "MISSING: $f"
...
echo "=================================================="
for f in src/services/CoordinationTraceService.ts src/services/MultiStrategySignalCoordinatorService.ts server.ts; do
  if [ -f "$f" ]; then
...
grep -n -B 5 -A 5 "coordination-trace" server.ts
echo ""
