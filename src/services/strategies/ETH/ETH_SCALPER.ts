...
export class ETHScalperStrategy implements BaseStrategy {
  validate(state: MarketAnalysisState, config: AppConfig) {
    ...
    const isDeepOversold = state.fisher < -0.3 || vwapZScore < -0.5; 
    const isDeepOverbought = state.fisher > 0.3 || vwapZScore > 0.5; 
    
    // Volume & Microstructure confirmation
    const isVolumeSpike = state.volRatio > 1.0;
    const orderFlowConfirmLong = state.liquidityGap > 0.05; // 5% more bids
    const orderFlowConfirmShort = state.liquidityGap < -0.05; // 5% more asks
    ...
    if (isDeepOversold) longScore += 40;
    if (isDeepOverbought) shortScore += 40;
